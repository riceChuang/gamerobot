...
// Base of stragedy
type BaiRenBase struct {
	*BaseGameType
}

// NewBase ...
func NewBaiRenBase(cfg model.CommonCfg, gamelogic gamehandler.GameLogicBase) GameType {
	fb := &BaiRenBase{
...
func (fb *BaiRenBase) Initialize(gameConnect *connecttype.GameConnect) {
...
func (fb *BaiRenBase) HandleMessage(message *model.WSMessage) {
...
func (fb *BaiRenBase) onUserStateChange(msg interface{}) {
	...
	if user.GetUserID() == fb.gameConnect.GetUserID() {
		...
		fb.gameHandler.SetUserMoney(fb.gameConnect.UserMoney)
