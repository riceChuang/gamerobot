...
// Login to get token
func (ds *DSGApiService) Login(loginDomain string, agentID int, account string, gameID int32) (string, error) {
	timestamp := time.Now().Unix() // s
	param := ds.getLoginParam(account, gameID)
	...
	if s, ok := jsonObj["s"]; ok {
		if d, ok := jsonObj["d"]; s.(float64) == 100 && ok {
			...
			if url, ok := d.(map[string]interface{})["url"]; ok {
...
func (ds *DSGApiService) getLoginParam(account string, gameID int32) string {
